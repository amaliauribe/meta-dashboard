...
// Microsoft Advertising Reporting API call
async function callReportingApi(reportRequest) {
    const token = await getAccessToken();
    
    const url = 'https://reporting.api.bingads.microsoft.com/Reporting/v13/GenerateReport';
    
    const soapEnvelope = buildSoapEnvelope(reportRequest, token);
    
    const response = await fetch(url, {
        method: 'POST',
        headers: {
            'Content-Type': 'text/xml; charset=utf-8',
            'SOAPAction': 'GenerateReport'
        },
        body: soapEnvelope
    });
...
// Build SOAP envelope for reporting request
function buildSoapEnvelope(request, token) {
...
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
  <s:Header>
    ...
    <h:AccountId xmlns:h="https://bingads.microsoft.com/Reporting/v13">${MSADS_CONFIG.accountId}</h:AccountId>
  </s:Header>
  <s:Body>
    ${request}
  </s:Body>
...
// Parse SOAP response (simplified - extracts report URL or data)
function parseReportResponse(xml) {
    // Extract report download URL from response
    const urlMatch = xml.match(/<ReportDownloadUrl[^>]*>([^<]+)<\/ReportDownloadUrl>/);
    if (urlMatch) {
        return { downloadUrl: urlMatch[1] };
    }
...
// Submit report request and poll for completion
async function submitAndDownloadReport(reportType, startDate, endDate, columns) {
    ...
    if (reportType === 'account') {
        reportRequest = buildAccountPerformanceReportRequest(startDate, endDate, columns);
    } else if (reportType === 'campaign') {
        reportRequest = buildCampaignPerformanceReportRequest(startDate, endDate, columns);
    } else {
        reportRequest = buildAccountPerformanceReportRequest(startDate, endDate, columns);
    }
    ...
    const submitEnvelope = `<?xml version="1.0" encoding="utf-8"?>
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
  <s:Header>
    <h:AuthenticationToken xmlns:h="https://bingads.microsoft.com/Reporting/v13">${token}</h:AuthenticationToken>
    <h:DeveloperToken xmlns:h="https://bingads.microsoft.com/Reporting/v13">${MSADS_CONFIG.developerToken}</h:DeveloperToken>
  ...
  <s:Body>
    ...
    while (!downloadUrl && attempts < maxAttempts) {
        ...
        const pollEnvelope = `<?xml version="1.0" encoding="utf-8"?>
<s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
  <s:Header>
    <h:AuthenticationToken xmlns:h="https://bingads.microsoft.com/Reporting/v13">${token}</h:AuthenticationToken>
    <h:DeveloperToken xmlns:h="https://bingads.microsoft.com/Reporting/v13">${MSADS_CONFIG.developerToken}</h:DeveloperToken>
...
// Build account performance report request
function buildAccountPerformanceReportRequest(startDate, endDate, columns) {
...
function buildCampaignPerformanceReportRequest(startDate, endDate, columns) {
    const columnElements = columns.map(c => `<Column>${c}</Column>`).join('\n            ');
...
app.post('/api/bing/account-performance', async (req, res) => {
    if (!isBingConfigured()) {
        return res.status(503).json({ error: 'Microsoft Ads credentials not configured. Please set environment variables.' });
    }
...
// Get daily performance (for charts and daily table)
