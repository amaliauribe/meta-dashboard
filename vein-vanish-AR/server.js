...
app.post('/api/process-image', upload.single('image'), async (req, res) => {
    try {
        ...
        // Convert any image format (HEIC, WEBP, etc.) to PNG for canvas compatibility
        const convertedPath = req.file.path + '_converted.png';
        await sharp(req.file.path)
            .png()
            .toFile(convertedPath);
        console.log('Converted to PNG:', convertedPath);
        ...
        // Call OpenAI Image Edit API (use toFile to ensure proper mimetype)
        const response = await openai.images.edit({
            model: "dall-e-2",
            image: await toFile(fs.readFileSync(imagePath), 'image.png', { type: 'image/png' }),
            mask: await toFile(fs.readFileSync(maskPath), 'mask.png', { type: 'image/png' }),
